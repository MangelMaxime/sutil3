...
let parseReference =
    (parseChar (fun c -> c >= 'A' && c <= 'Z') "col") .>>. parseNatural
    |> map Reference

let parseMulOp =
    ignoreWhite (parseCharLit '*' <|> parseCharLit '/' <|> parseCharLit '%')
...
let rec parseFactor = ignoreWhite (parseNumber <|> parseReference <|> parseSubExpr)
...
and parseMulExpr =
    let inner input =
        let p =
            parseFactor .>>. ((parseMulOp .>>. parseFactor) |> parseSequence |> optional)
...
and parseAddExpr =
    let inner input =
        let p =
            parseMulExpr .>>. ((parseAddOp .>>. parseMulExpr) |> parseSequence |> optional)

        run p input |> buildBinTree

    Parser inner

and parseExpr = ignoreWhite parseAddExpr

// Handle '= ..." expressions and treat everything else as a numeric literal
let parseCellExpr = (parseCharLit '=' ..>. parseExpr) <|> parseNumber
...
let parseToEnd p =
    let inner input =
        run p input
        >>= (fun (e, r) ->
            if r |> skipWhite |> atEnd then
                Success(e, r)
            else
                ParseResult.Error $"Syntax error at {r}"
        )
...
let rec evalExpr (sheet: Position -> string) e =
    ...
    | Binary(a, op, b) ->
        ...
        (evalExpr sheet a)
        >>= (fun av -> evalExpr sheet b >>= (fun bv -> Success(ops.[op] av bv)))
    | Reference pos ->
...
let evalCellAsString sheet content =
    let result =
        run (parseToEnd parseCellExpr) content >>= (fun (e, r) -> evalExpr sheet e)

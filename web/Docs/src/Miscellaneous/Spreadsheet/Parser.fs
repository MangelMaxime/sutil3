...
module ParseResult =
    ...
    [<CompiledName("MapSnd")>]
    let mapSnd f self = self |> map (fun (a, b) -> (a, b |> f))
...
let andThen first second =
    let inner input =
        ...
        | Success(value1, input') ->
            ...
            match result2 with
            | Error msg -> Error msg
            | Success(value2, input'') -> Success((value1, value2), input'')
...
let andThenDiscardFirst first second =
    let inner input =
        ...
        | Success(_, input') ->
            ...
            match result2 with
            | Error msg -> Error msg
            | Success(value2, input'') -> Success(value2, input'')
...
let andThenDiscardSecond first second =
    let inner input =
        ...
        | Success(value1, input') ->
            ...
            match result2 with
            | Error msg -> Error msg
            | Success(_, input'') -> Success(value1, input'')
...
let parseDigit = parseChar Char.IsDigit "digit"

let parseWhite = parseChar Char.IsWhiteSpace "digit"
...
let choose parsers = parsers |> List.reduce (<|>)
...
let parseInteger =
    (optional (parseChar (isChar '-') "negate")) .>>. (parseSequence parseDigit)
...
let parseString =
    let inner input =
        ...
        let p =
            (parseChar q "quote") ..>. (parseSequence (parseChar nq "string character"))
            .>.. (parseChar q "quote")
        ...
        match r with
        | Error msg -> Error msg
        | Success(chars, remainder) -> Success(String(List.toArray chars), remainder)
...
let parseIdent =
    let inner input =
        let p =
            (parseChar Char.IsLetter "letter")
            .>>. (parseSequence (parseChar Char.IsLetterOrDigit "ident character"))
...
let ignoreWhite p =
    (optional (parseSequence parseWhite)) ..>. p

...
module Random =
    let shuffleR xs =
        xs |> Seq.sortBy (fun _ -> int (Interop.random () * 100.0))
...
type DataSimulation =
    static member Stream (init: unit -> 'T) (f: int -> 'T -> 'T) (delay: int) =
        ...
        let store = Store.makeStore init (fun _ -> dispose ())

        dispose <-
            Timers.interval
                (fun _ ->
                    tick <- tick + 1
                    Store.modify (fun v -> f tick v) store
                )
                delay
    ...
    static member Records
        (data: 'T list, update: 'T -> 'T, maxEditsPerTick: int, allowCreateDelete: bool, delay: int)
        : IStore<'T list>
        =
        ...
        let edit _ current =
            ...
            |> List.fold (fun current' _ -> editOne current') current
    ...
    static member Random(min: int, max: int, delay: int) =
        let next () =
            min + int (System.Math.Round(float (max - min) * Interop.random ()))

        DataSimulation.Stream next (fun _ _ -> next ()) delay

    static member Random(max: int) = DataSimulation.Random(1, max, 1000)
...
module SampleData =
    ...
    let updateStock (r: Stock) =
        { r with
            Price = r.Price * (1.0 + randomSign (0.01))
        }
    ...
    let sampleStocks (number: int) =
        let nextSymbol () =
            ...
            |> shuffleR
            |> Seq.take 4
            |> Seq.fold (fun s c -> sprintf "%s%c" s c) ""
        ...
        |> List.map (fun _ ->
            {
                Symbol = nextSymbol ()
                Price = System.Math.Round(Interop.random () * 100.0, 2)
            }
        )

    let stockFeed numStocks delay =
        DataSimulation.Records(sampleStocks (numStocks), updateStock, 5, false, delay)
    ...
    let sampleTodos () =
        [
            ...
            {
                Description = "DevTools for Firefox"
                Completed = true
            }
            {
                Description = "DevTools for Chrome"
                Completed = false
            }
            {
                Description = "Implement benchmarks"
                Completed = false
            }
            {
    ...
    let updateTodo (r: Todo) =
        { r with
            Completed = not r.Completed
        }

    let todosFeed numTodos delay =
        DataSimulation.Records(sampleTodos () |> List.take numTodos, updateTodo, 1, true, delay)

...
open Spreadsheet.Evaluator
open System

let log s = () // Fable.Core.JS.console.log(s)
let formatPosition (p: Position) = sprintf "[%A:%A]" (fst p) (snd p)

let formatPositions (positions: Position seq) =
    "[" + (positions |> Seq.map formatPosition |> String.concat ",") + "]"
...
let nodeOfCell pos =
    Browser.Dom.document.querySelector ($"[x-id='{positionStr pos}'") :?> Browser.Types.HTMLElement
...
module Cells =
    ...
    let private cellCollectCells pos =
        let mutable cells: Map<Position, int> =
            Map [
                (pos, 1)
            ]
...
let update (message: Message) (model: Model) : Model * Cmd<Message> =
    ...
    | RefreshCell p ->
        ...
        Cmd.none //[fun d -> Cells.cellNotify p (d <<RefreshCell)]
...
let view () : SutilElement =
    ...
    let renderPlainCell pos =
        ...
        Html.fragment [
            Ev.onClick (fun _ -> StartEdit pos |> dispatch)
    ...
    let renderActiveCell pos =
        ...
        Html.div [
            Html.input [
                ...
                Ev.onBlur (fun me -> (pos, me.target?value) |> UpdateValue |> dispatch)
            ]
        ]
    ...
    Html.div [
        let rowsXcols =
            Observable.zip (model .> rows) (model .> cols)
        ...
        Bind.el (
            ...
            fun (rows, cols) ->
                Html.table [
                    ...
                    let activeS =
                        (model |> Observable.map (fun m -> m.Active) |> Observable.pairwise)

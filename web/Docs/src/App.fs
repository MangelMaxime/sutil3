...
let currentBook m =
    match m.View with
    | PageView v -> Some v.Book
    | _ -> None

let currentPage m =
    match m.View with
    | PageView v -> Some v.Page
    | _ -> None

let currentSection m =
    match m.View with
    | PageView v -> Some v.Section
    | _ -> None
...
let makeHref book page section =
    ...
    + (sanitize book
       + "-"
       + sanitize page
       + (if page = section || section = "" then
              ""
          else
              "?" + section))

let makeBookHref bk =
    makeHref bk.Title bk.defaultPage.Title ""

let findPage bk title =
    bk.Pages |> List.tryFind (fun p -> sanitize p.Title = sanitize title)
...
let pageCategories (all: Page list) =
    all |> List.map (fun p -> p.Category) |> List.distinct

let compareBook (a: BookPageView) (b: BookPageView) = a.Book.Title = b.Book.Title
...
let update msg model : Model * Cmd<Message> =
    ...
    | AddBook book ->
        let cmd =
            model.ViewRequest
            |> Option.map (Cmd.ofMsg << SetPageView)
            |> Option.defaultValue Cmd.none

        { model with
            Books = book :: model.Books
        },
        cmd
        ...
        if request.BookName = "" then
            ...
            let section =
                ...
                | AppLink(creator, sections) ->
                    if (sections |> List.contains request.SectionName) then
                        request.SectionName
                    else
                        ""
                | Url _ -> ""
            ...
            { model with
                ...
                ShowContents = false
                ViewRequest = Some request
            },
            cmd

    | ToggleShowContents ->
        { model with
            ShowContents = not model.ShowContents
        },
        Cmd.none
...
let Section (tab: Book) (name: string) =
    Html.fragment [
        Html.h5 [
            Attr.className "title is-6"
            text (name.ToUpper())
        ]
        Html.ul [
            for page in tab.Pages |> List.filter (fun x -> x.Category = name) do
                Html.li [
                    Html.a [
                        match page.Link with
                        | AppLink(_, _) -> Attr.href <| makeHref tab.Title page.Title ""
...
let viewPage (view: System.IObservable<BookPageView>) =
    ...
    let sectionView =
        view |> Observable.distinctUntilChangedCompare compareSectionSource
        ...
        Bind.el (
            ...
            fun bpv ->
                ...
                | "" ->
                    try
                        match page.Link with
                        | AppLink(creator, sections) -> creator ()
...
let viewBook showContents (bookPageView: System.IObservable<BookPageView>) =
    ...
    Html.div [
        ...
        Html.div [
            ...
            book.Pages
            |> pageCategories
            |> List.map (fun title -> Section book title)
            |> Html.div
        ] //|> showContents
...
let tiles =
    [
        ...
        Html.div [
            ...
            articleTile "color-2" "Awesome Transitions" "One-liner transitions"
            articleTile "color-3" "Easy DOM" "Build content with code-completion and tooltips"
        ]
        ...
        Html.div [
            ...
            articleTile "color-3" "Fully Reactive" "Connect your data directly to your view"
            articleTile "color-1" "Integration Friendly" "Plays well with other frameworks"
        ]
        Html.div [
            Attr.className "tile is-ancestor"
            Attr.style [
    ...
    Html.div [
        ...
        Html.div [
                ...
                Bind.el (
                    ...
                    fun books ->
                        Html.span [
                            ...
                            Html.a [
                                text "REPL"
                                Attr.href "https://sutil.dev/repl"
                            ]
                        ]
                )
                ...
                Html.fragment [
                    ...
                    Html.a [
                        ...
                        Html.img [
                            ...
                            Attr.src
                                "https://img.shields.io/nuget/vpre/Sutil?color=rgb%2861%2C80%2C80%29&label=%20nuget&style=social"
                        ]
                    ]
                ]
            ...
            Html.a [
                ...
                Attr.href "#"
                Html.i [
                    Attr.className "fa fa-bars"
                ]
                Ev.onClick (fun ev ->
                    ev.preventDefault ()
                    ToggleShowContents |> dispatch
                )
            ]
        ]
        ...
        let keyForView =
            function
            | FrontPage -> "FrontPage"
            | PageView v -> v.Book.Title

        Bind.el (
            "page",
            model |> Store.map getView,
            keyForView,
            fun keyedView ->
...
let main () = appMain () |> Program.mount

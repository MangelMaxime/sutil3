...
let viewItemO (r: System.IObservable<Record>) = Bind.el (r, viewItem)

describe "Sutil.Binding"
<| fun () ->

    it "Binding has minimal required elements"
    <| fun () ->
        promise {
    ...
    it "Binding allows change in element tag"
    <| fun () ->
        promise {
            let data = Store.make 0
    ...
    it "Shows exception if binding fails"
    <| fun () ->
        promise {
            let data = Store.make 0
            let app = Bind.el (data, fun n -> failwith "expected-exception")
            mountTestApp app
    ...
    // can re-order
    it "Doesn't dispose items when re-ordering"
    <| fun () ->
        promise {
                        ...
                        (fun (n: int) ->
                            Html.div [
                                unsubscribeOnUnmount [
                                    fun _ -> unmountCount <- unmountCount + 1
                                ]
                                text (string n)
                            ]
                        ),
    ...
    <| fun () ->
        promise {
            ...
            Expect.areEqual (
                Store.countSubscribers storeInner,
                1,
                "1 inner subscriber before store update"
            )

            Expect.areEqual (
                Store.countSubscribers storeOuter,
                1,
                "1 outer subscriber before store update"
            )
            ...
            Expect.areEqual (
                Store.countSubscribers storeInner,
                1,
                "1 inner subscriber after store update"
            )

            Expect.areEqual (
                Store.countSubscribers storeOuter,
                1,
                "1 outer subscriber after store update"
            )
    ...
    <| fun () ->
        promise {
            ...
            let reset () = numRenders <- 0

            let render () = numRenders <- numRenders + 1

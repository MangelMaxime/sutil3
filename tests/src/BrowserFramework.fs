...
let timeNow () =
    (float) System.DateTime.Now.Ticks / 10000000.0

let timestamp (t: float) = sprintf "%0.3f" t
...
let waitAnimationFrame () : JS.Promise<unit> =
    Fable.Core.JS.Constructors.Promise.Create(fun accept _ -> rafu accept)
...
let testList (name: string) (tests: TestCase list) =
    {
        Name = name
        Tests = tests
        Suites = []
...
let rec nextTestCase (testCtx: TestContext) = rafu (fun _ -> nextTestCaseNow testCtx) // Allow browser to update between tests
    ...
    | test :: tests ->
        ...
        |> Promise.map (fun _ ->
            ...
            nextTestCase
                { testCtx with
                    TestCases = tests
                    NumPass = testCtx.NumPass + 1
                }
        )
        |> Promise.catch (fun x ->
            logE (
                $"{timestamp (timeNow () - testCtx.StartTime)}: "
                + test.Name
                + ": FAIL: "
                + x.Message
            )
...
and nextTestSuite (testCtx: TestContext) =
    ...
    | [] ->
        ...
        if testCtx.NumFail = 0 then
            logS (
                $"{timestamp (timeNow () - testCtx.StartTime)}: SUCCESS: All tests passed ("
                + string testCtx.NumPass
                + ")"
            )
        else
            logE (
                $"{timestamp (timeNow () - testCtx.StartTime)}: FAIL: "
                + string testCtx.NumFail
                + " failed, "
                + string testCtx.NumPass
                + " passed"
            )
...
let runAll (suiteName: string, caseName: string) =
    runTests (
        ...
        |> List.map (fun s ->
            { s with
                Tests = s.Tests |> List.filter (fun t -> caseName = "*" || t.Name = caseName)
            }
        )
    )

...
describe "Sutil.Store"
<| fun () ->

    it "Immediate initialisation upon subscription"
    <| fun () ->
        promise {
            let s = Store.make 42
            let mutable v = 0
    ...
    it "Store updates when no subscribers"
    <| fun () ->
        promise {
            let s = Store.make 42
            Store.set s 43
    ...
    it "Multiple subscribers initialize"
    <| fun () ->
        promise {
            let s = Store.make 42
    ...
    it "Multiple subscribers update"
    <| fun () ->
        promise {
            let s = Store.make 42
    ...
    it "Dispose terminates subscription"
    <| fun () ->
        promise {
            let s = Store.make 42
    ...
    it "No dispose on number of subscribers becoming 0"
    <| fun () ->
        promise {
            let s = Store.make 42

...
let inline private _element tag children = Sutil.Basic.el tag children
let inline private _fragment children = Sutil.Basic.fragment children
let inline private _text s = Sutil.Basic.text s
let inline private _attr (name: string, value: obj) = Sutil.Basic.attr name value
let inline private _ev (event: string, handler: Event -> unit) = Sutil.Basic.event event handler

let inline private _evx
    (event: string, handler: Event -> unit, options: CustomEvents.EventOption[])
    =
    Sutil.Basic.eventWithOptions event handler options

// Dummy type to avoid problems with overload resolution in HtmlEngine
///<exclude/>
[<Fable.Core.Erase>]
type NodeAttr = NodeAttr of SutilElement
...
type SutilEventEngine() =
    inherit EventEngine<SutilElement>(fun (event: string) handler -> _ev (event, handler))

    member __.onCustomEvent<'T>(event: string, handler: CustomEvent<'T> -> unit) =
        _ev (event, !!handler)

    member __.onMouseMove(handler: MouseEvent -> unit) = _ev ("mousemove", !!handler)

    member __.onMouseOver(handler: MouseEvent -> unit) = _ev ("mouseover", !!handler)

    member __.onMount(handler) = _ev (CustomEvents.MOUNT, handler)

    member __.onUnmount(handler) = _ev (CustomEvents.UNMOUNT, handler)

    member __.onInputT(handler: TypedEvent<HTMLInputElement> -> unit) = _ev ("input", !!handler)
...
type SutilHtmlEngine() as this =
    inherit HtmlEngine<SutilElement>(_element, _text, (fun () -> _fragment []))
    ...
    member __.sectionc (cls: string) (children: seq<SutilElement>) =
        __.section (_clsch cls children)
    ...
    member __.articlec (cls: string) (children: seq<SutilElement>) =
        __.article (_clsch cls children)
    ...
    member _.text(v: IObservable<string>) = Bind.el (v |> Store.distinct, _text)

    member _.text(v: IObservable<int>) =
        Bind.el (v .> string |> Store.distinct, _text)

    member _.text(v: IObservable<float>) =
        Bind.el (v .> string |> Store.distinct, _text)

    member _.td(v: IObservable<string>) = Bind.el (v |> Store.distinct, this.td)

    member _.td(v: IObservable<int>) =
        Bind.el (v .> string |> Store.distinct, this.td)

    member _.td(v: IObservable<float>) =
        Bind.el (v .> string |> Store.distinct, this.td)

    member _.span(v: IObservable<string>) =
        Bind.el (v |> Store.distinct, this.span)

    member _.span(v: IObservable<int>) =
        Bind.el (v .> string |> Store.distinct, this.span)

    member _.span(v: IObservable<float>) =
        Bind.el (v .> string |> Store.distinct, this.span)

    member _.h1(v: IObservable<string>) = Bind.el (v |> Store.distinct, this.h1)

    member _.h1(v: IObservable<int>) =
        Bind.el (v .> string |> Store.distinct, this.h1)

    member _.h1(v: IObservable<float>) =
        Bind.el (v .> string |> Store.distinct, this.h1)

    member _.h2(v: IObservable<string>) = Bind.el (v |> Store.distinct, this.h2)

    member _.h2(v: IObservable<int>) =
        Bind.el (v .> string |> Store.distinct, this.h2)

    member _.h2(v: IObservable<float>) =
        Bind.el (v .> string |> Store.distinct, this.h2)

    member _.h3(v: IObservable<string>) = Bind.el (v |> Store.distinct, this.h3)

    member _.h3(v: IObservable<int>) =
        Bind.el (v .> string |> Store.distinct, this.h3)

    member _.h3(v: IObservable<float>) =
        Bind.el (v .> string |> Store.distinct, this.h3)

    member _.h4(v: IObservable<string>) = Bind.el (v |> Store.distinct, this.h4)

    member _.h4(v: IObservable<int>) =
        Bind.el (v .> string |> Store.distinct, this.h4)

    member _.h4(v: IObservable<float>) =
        Bind.el (v .> string |> Store.distinct, this.h4)

    member _.h5(v: IObservable<string>) = Bind.el (v |> Store.distinct, this.h5)

    member _.h5(v: IObservable<int>) =
        Bind.el (v .> string |> Store.distinct, this.h5)

    member _.h5(v: IObservable<float>) =
        Bind.el (v .> string |> Store.distinct, this.h5)

    member _.h6(v: IObservable<string>) = Bind.el (v |> Store.distinct, this.h6)

    member _.h6(v: IObservable<int>) =
        Bind.el (v .> string |> Store.distinct, this.h6)

    member _.h6(v: IObservable<float>) =
        Bind.el (v .> string |> Store.distinct, this.h6)

    member _.div(v: IObservable<string>) = Bind.el (v |> Store.distinct, this.div)

    member _.div(v: IObservable<int>) =
        Bind.el (v .> string |> Store.distinct, this.div)

    member _.div(v: IObservable<float>) =
        Bind.el (v .> string |> Store.distinct, this.div)

    member _.fragment(v: seq<SutilElement>) = _fragment v
...
type SutilAttrEngine() =
    ...
    member _.value<'T>(value: IObservable<'T>, dispatch: 'T -> unit) =
        Bind.attr ("value", value, dispatch)

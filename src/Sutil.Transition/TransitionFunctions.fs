...
let private log = Log.create "trfn"
...
let internal computedStyleOpacity e =
    ...
    with _ ->
        if log.enabled then
            log.error (sprintf "parse error: '%A'" (window.getComputedStyle(e).opacity))
...
let fade (initProps: TransitionProp list) (node: HTMLElement) =
    fun () ->
        ...
        { tr with
            CssGen = Some(fun t _ -> sprintf "opacity: %f" (t * computedStyleOpacity node))
        }

let slide (props: TransitionProp list) =
...
let draw (props: TransitionProp list) =
    fun (node: SVGPathElement) ->
        let tr =
            applyProps
                props
                { Transition.Default with
    ...
    fun () ->
        {
          ...
          tr with
            CssGen =
                Some(fun t u ->
                    sprintf
                        "transform: %s translate(%fpx, %fpx); opacity: %f;"
                        transform
                        ((1.0 - t) * tr.X)
                        ((1.0 - t) * tr.Y)
                        (targetOpacity - (od * u))
...
let crossfade (userProps: TransitionProp list) =
    ...
    let crossfadeInner (from: ClientRect, node: Element, props, intro) =
        let tr =
            { Transition.Default with
                Ease = Easing.cubicOut
                DurationFn = Some(fun d -> System.Math.Sqrt(d) * 30.0)
            }
            |> applyProps props
    ...
    let transition
        (
            items: Dictionary<string, ClientRect>,
            counterparts: Dictionary<string, ClientRect>,
            intro: bool
        )
        =
    ...
    (transition (toSend, toReceive, false), transition (toReceive, toSend, true))

...
[<Literal>]
let private TICK_TASK = "__sutil_ticktask"
...
let internal showEl (el: HTMLElement) isVisible =
    if isVisible then
        if JsMap.hasKey el "_display" then
            addStyleAttr el "display" (JsMap.getKey el "_display")
        else
...
module private LoopTasks =
     ...
     *)
    let private clearLoops = tasks.Clear()
...
and CreateTransition = unit -> Transition
...
let mergeProps newerProps existingProps : TransitionProp list = existingProps @ newerProps

let withProps
    (userProps: TransitionProp list)
    (f: TransitionProp list -> 'a)
    : TransitionProp list -> 'a
    =
    fun (initProps: TransitionProp list) -> initProps |> mergeProps userProps |> f
...
let private overrideDuration d =
    if slowAnimations then
        10.0 * d
    else
        d

let private overrideDurationFn fo =
    if slowAnimations then
        (fo |> Option.map (fun f -> ((*) 10.0 << f)))
    else
        fo
...
let private registerDoc (doc: Browser.Types.Document) =
    ...
    if log.enabled then
        log.info ($"Active docs: {activeDocs.Count}")
...
let private runTasks () =
    ...
    if (copy.Length > 0) then
        if log.enabled then
            log.info ($"- - - Tasks: running {copy.Length} tasks - - - - - - - - - - - - - -")
...
let createRule (node: HTMLElement) (a: float) (b: float) tr (uid: int) =
    ...
    if log.enabled then
        log.info (
            sprintf "keyframe: %s" (keyframes |> List.skip (keyframes.Length / 2) |> List.head)
        )

    if log.enabled then
        log.info ($"createRule {name} {durn}ms for {nodeStr node}")

    let stylesheet = getSutilStylesheet (documentOf node)
    stylesheet.insertRule (keyframeText, stylesheet.cssRules.length) |> ignore

    let animations =
        if String.IsNullOrEmpty(node.style.animation) then
            []
        else
            [
    ...
    Timers.rafu (fun _ ->
        if (numActiveAnimations = 0) then
            for kv in activeDocs do
                ...
                if log.enabled then
                    log.info (sprintf "clearing %d rules" (int stylesheet.cssRules.length))

                for i in
                    [
                        (int stylesheet.cssRules.length - 1) .. -1 .. 0
                    ] do
    ...
    let next =
        ...
        |> Array.filter (
            if System.String.IsNullOrEmpty(name) then
                (fun anim -> anim.IndexOf(name) < 0) // remove specific animation
            else
                (fun anim -> anim.IndexOf("__sutil") < 0)
...
let flip props (node: Browser.Types.Element) (animation: Animation) =
    ...
    if log.enabled then
        log.info (
            sprintf
                ...
                dy
                transform
                (rectToStr animation.From)
                (rectToStr animation.To)
        )
    ...
    { tr with
        ...
        CssGen =
            Some(fun t u ->
                sprintf "transform: %s translate(%fpx, %fpx);`" transform (u * dx) (u * dy)
            )
    }
...
let createAnimation
    ...
    let shouldCreate =
        ...
        && not (
            from.left = tgt.left
            && from.right = tgt.right
            && from.top = tgt.top
            && from.bottom = tgt.bottom
        )
    //    return noop;
    ...
    if (shouldCreate) then
        ...
        let r =
            { a with
                Duration =
                    if (a.Duration = 0.0 && a.DurationFn.IsNone) then
                        300.0
                    else
                        a.Duration
            }
...
let private tickGen = Helpers.createIdGenerator ()

let private findTransition
    (intro: bool)
    (trans: TransitionAttribute list)
    : TransitionBuilder option
    =
    let mutable result: TransitionBuilder option = None

    for x in trans do
        result <-
            match result, x, intro with
            | Some _, _, _ -> result
...
let transitionNode
    ...
    let cancelTick () =
        match JsMap.tryGetKey el TICK_TASK with
        | Some f ->
            JsMap.deleteKey el TICK_TASK
            f ()
        | None -> ()

    let runTick tr b durn =
        // let logEnabled = Log.isSourceEnabled "tick"
        ...
        let tick =
            match tr.Tick with
            | Some f -> f
            | None -> failwith "No tick function supplied"
    ...
    let hide () =
        // if log.enabled then log $"hide {nodeStr el}"
        showEl el false
        complete el
        ...
        CustomEvents.CustomDispatch<_>.dispatch (el, "outroend")

    let show () =
        // if log.enabled then log $"show {nodeStr el}"
        showEl el true
        complete el
        ...
        CustomEvents.CustomDispatch<_>.dispatch (el, "introend")
    ...
    let startTransition createTrans =
        ...
        waitAnimationFrame
        <| fun () ->
            CustomEvents.CustomDispatch<_>.dispatch (el, event)
            start el
            ...
            if tr.DurationFn.IsSome then
                failwith "Duration function not permitted"
    ...
    if not (isNull _display) && _display <> "" && _display <> "none" then
        JsMap.setKey el "_display" _display

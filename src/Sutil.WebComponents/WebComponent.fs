    ...
    let adoptStyleSheet (styleSheet: Sutil.Styling.Types.SutilStyleRule list) =
        CoreElements.onElementMounted (fun el ->
            ...
            let run () =
                ...
                sheet.replaceSync (
                    styleSheet |> SutilStyleSheet.Of |> Sutil.Styling.Renderer.styleSheetAsText
                )
    ...
    static member Register<'T>
        ...
        let wrapper (host: Node) : WebComponent.Callbacks<'T> =
...

            {
                ...
                OnConnected =
                    fun _ ->
                        CustomEvents.CustomDispatch<_>
                            .dispatch (
                                (host.shadowRoot.firstChild) :> EventTarget,
                                CustomEvents.CONNECTED
                            ) //"sutil-connected"
            }

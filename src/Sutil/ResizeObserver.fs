...
[<Literal>]
let private RESIZE_KEY = "__sutil_resize"
...
type ResizeObserver(el: HTMLElement) =
    ...
    do
        ...
        let style =
            sprintf
                "%sz-index: %i;"
                "display: block; position: absolute; top: 0; left: 0; width: 100%; height: 100%; overflow: hidden; border: 0; opacity: 0; pointer-events: none;"
                zIndex
        ...
        if isCrossOrigin then
            ...
            unsubscribe <-
                EventListeners.add
                    ...
                    window
                    (fun e ->
                        if JsHelpers.eq3 (JsMap.getKey e "source") iframe.contentWindow then
                            notify (e)
                    )
        else
    ...
    member _.Subscribe(callback: (unit -> unit)) =
        ...
        (fun () -> subscribers <- subscribers |> List.filter (fun s -> s.Id <> sub.Id))

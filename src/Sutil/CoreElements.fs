...
let unsubscribeOnUnmount (fns: (unit -> unit) seq) =
    onElementMounted (fun el -> fns |> Seq.iter (Dispose.addUnsubscribe el "unsubscribeOnUnmount"))
...
let disposeOnUnmount (fns: (System.IDisposable) seq) =
    onElementMounted (fun el -> fns |> Seq.iter (Dispose.addDisposable el "disposeOnUmount"))
...
let bindDisposable<'Element when 'Element :> HTMLElement>
    (handler: 'Element -> System.IDisposable)
    =
    onElementMounted (fun el -> Dispose.addDisposable el "bindSubEl" (handler el))
...
let bindSubscribe<'T, 'Element when 'Element :> HTMLElement>
    (source: System.IObservable<'T>)
    (handler: 'Element -> 'T -> unit)
    =
...
let html (text: string) : SutilElement =
    SutilElement.DefineBinding(
        ...
        fun ctx ->
            ...
            host |> Node.children |> Seq.iter (ctx.NotifyNodeImported)
...
let postProcessElementsWithName
    ...
    SutilElement.DefineMapping(
        ...
        (fun context ->
            context.WithOnImportedNode(fun (node: Node) ->
...
let subscribe (source: System.IObservable<'T>) (handler: 'T -> unit) =
    onElementMounted (fun el -> source.Subscribe(handler) |> Dispose.addDisposable el "subscribe")
...
let inject (elements: SutilElement seq) (element: SutilElement) = append elements element
